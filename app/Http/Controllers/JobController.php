...
class JobController extends Controller
{
   ...
   /* public function index()
    ...
    public function index()
    {
        ...
        //this used in expire email for admin
...
try{
      ...
      if($company_status == 1)
 {
  $post_and_id = explode('**', $request['api_post']);
            ...
            $merit_array = (json_decode(CurlImpersonated::merites($token , $post_and_id[2],$baseurl)))->data;
            ...
            foreach ($merit_array as $merit)
            {
                ...
                $gng_generalmerit_array = array_combine($gng_generalmerit_name,$gng_generalmerit_value);
                $gng_professionalmerit_array = array_combine($gng_professionalmerit_name,$gng_professionalmerit_value);

                $general_skill = $request['skill_1'];

                foreach ($general_skill as $item) {
                if(array_key_exists($item['name'],$gng_generalmerit_array) ){$general_id = $gng_generalmerit_array[$item['name']];
                    $general_name = $item['name'];
                }
                ...
                $data=array('name' => $general_name,'general_data_id'=>$general_id ,'created_at'=>$date,'updated_at'=>$date);
                 DB::table('job_general_merites')->insert($data);
                ...
                $special_skill = $request['skill_2'];
                foreach ($special_skill as $item) {
                if(array_key_exists($item['name'],$gng_professionalmerit_array) ){$professional_id = $gng_professionalmerit_array[$item['name']];
                ...
                $data=array('name' => $professional_name,'general_data_id'=>$professional_id ,'created_at'=>$date,'updated_at'=>$date);
                 DB::table('job_professional_merites')->insert($data);
                ...
                $job->job_professional_merites()->attach([$merit->id => ['value' => $item['value']]]);
                      ...
                      //  die(json_encode($gng_professionalmerit_array));
...
die(json_encode($user_companies));*/
        ...
        function reminder_email()
    {
     ...
     public function acceptWithouotRelease($id)
    {
        $job = Job::findOrFail($id);
    ...
    {
        $pre_job = Job::where('id',$id)->first();

        if($pre_job->ItemId >0)
        {
            
            return redirect()->back()
		        ->withErrors('کپی کردن آگهی های مجوز جذب امکان پذیر نیست');
        ...
        $date = date('Y-m-d H:i:s');
                ...
                $new_j->company_id=$pre_job->company_id;
        $new_j->title=$pre_job->title;
        $new_j->alias=$pre_job->alias;
        $new_j->persian_alias=$pre_job->persian_alias;
        $new_j->expire_date=$pre_job->expire_date;
        $new_j->approved=$pre_job->approved;
        $new_j->apply_limit=$pre_job->apply_limit;
        $new_j->pin_status=$pre_job->pin_status;
        $new_j->department_id=$pre_job->department_id;
        $new_j->created_by=Auth::user()->id;
        $new_j->modified_by=Auth::user()->id;
        //$new_j->created_at=$pre_job->created_at;
        //$new_j->updated_at=$pre_job->updated_at;
        //$new_j->deleted_at=$pre_job->deleted_at;
        $new_j->post_id=$pre_job->post_id;
        $new_j->general_merites=$pre_job->general_merites;
        $new_j->professional_merites=$pre_job->professional_merites;
        $new_j->gender=$pre_job->gender;
        $new_j->province_id=$pre_job->province_id;
        $new_j->city_id=$pre_job->city_id;
        $new_j->industry_id=$pre_job->industry_id;
        $new_j->sort_order=$pre_job->sort_order;
        $new_j->min_education=$pre_job->min_education;
        $new_j->goal_or_mission=$pre_job->goal_or_mission;
        $new_j->main_responsibilities=$pre_job->main_responsibilities;
        $new_j->job_other_features=$pre_job->job_other_features;
        $new_j->status=$pre_job->status;
        $new_j->LatLng=$pre_job->LatLng;
        $new_j->cooperation_type=$pre_job->cooperation_type;
        $new_j->visit_count=$pre_job->visit_count;
        $new_j->jobExp=$pre_job->jobExp;
        $new_j->field=$pre_job->field;
        $new_j->admin_message=$pre_job->admin_message;
        $new_j->extra_question=$pre_job->extra_question;
        $new_j->PortalAddress=$pre_job->PortalAddress;
        $new_j->ItemId=$pre_job->ItemId;
        $new_j->PortalStatus=$pre_job->PortalStatus;
        $new_j->api_post_id=$pre_job->api_post_id;
        $new_j->api_department_id=$pre_job->api_department_id;
        $new_j->api_field_type_id=$pre_job->api_field_type_id;
        $new_j->api_field_id=$pre_job->api_field_id;   
        $new_j->save();
        
        $new_jobid = $new_j->id ; 
        
       
        $quiestions =  JobQuestions::where('job_id',$id)->get();
        if(count($quiestions) > 1 )
         {
             foreach($quiestions as $quiestion)
             {
                $q = new JobQuestions();
                $q->question = $quiestion->question;
                $q->job_id = $new_jobid;
                $q->save();
             }
             
         }
         else if(count($quiestions) == 1)
         {
                $q = new JobQuestions();
                $q->question = $quiestions->question;
                $q->job_id = $new_jobid;
                $q->save();
         }
         
        $job_cities = $results = DB::select('select * from  job_has_cities where job_id="'.$id.'"');

        if(count($job_cities) > 1 )
         {
             foreach($job_cities as $job_city)
             {
                DB::table('job_has_cities')->insert(
                ['job_id' => $new_jobid, 'city_id' => $job_city->id,'created_at'=>$date,'updated_at'=>$date]
            );
             }
             
         }
         else if(count($job_cities) == 1)
         {
               DB::table('job_has_cities')->insert(
                ['job_id' => $new_jobid, 'city_id' => $job_cities[0]->city_id,'created_at'=>$date,'updated_at'=>$date]
            );
         }
       
       
        $general_skills = $results = DB::select('select * from  job_has_general_merites where job_id="'.$id.'"');

        if(count($general_skills) > 1 )
         {
             foreach($general_skills as $general_skill)
             {
                DB::table('job_has_general_merites')->insert(
                ['job_id' => $new_jobid, 'general_merites_id' => $general_skill->general_merites_id, 'value' => $general_skill->value,'created_at'=>$date,'updated_at'=>$date]
            );
             }
             
         }
         else if(count($general_skills) == 1)
         {
            DB::table('job_has_general_merites')->insert(
                ['job_id' => $new_jobid, 'general_merites_id' => $general_skills->general_merites_id, 'value' => $general_skills->value,'created_at'=>$date,'updated_at'=>$date]
            );
         }
         
         
        $professional_skills = $results = DB::select('select * from  job_has_professional_merites where job_id="'.$id.'"');

        if(count($general_skills) > 1 )
         {
             foreach($professional_skills as $professional_skill)
             {
                DB::table('job_has_professional_merites')->insert(
                ['job_id' => $new_jobid, 'professional_merites_id' => $professional_skill->professional_merites_id, 'value' => $professional_skill->value,'created_at'=>$date,'updated_at'=>$date]
            );
             }
        ...
        return \Redirect::route('jobs.edit', ['id'=>$new_jobid]);
