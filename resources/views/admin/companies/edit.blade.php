    ...
    <div class="col-xs-12 wrapper wrapper-content animated fadeInLeft white-bg">
        <div class="row">
            <div class="col-lg-6">
                ...
                {{ Form::open(array('route' => ['companies.update',$company->id] , 'method'=>'put')) }}
            ...
            <div class="form-group col-lg-12">
                ...
                <select name="gig_company_id" id="admins" data-placeholder="انتخاب کنید" class="form-control chosen">
                    @foreach($gig_all_companies as $item)
                        <option @if($company->gig_company_id ==$item['id'] ) selected @endif value="{{$item['id']}}">{{$item['title']}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group col-lg-12">
                ...
                <select name="admins[]" multiple id="admins" data-placeholder="انتخاب کنید" class="form-control chosen">
                    @foreach($users as $user)
                        <option @if(in_array($user->id,$company->users->pluck('id')->toArray())) selected
            ...
            <div class="form-group-lg col-lg-6">
                <label for="thumbnail3">گالری تصاویر شرکت</label>
            ...
            <textarea id="selected_images_field" name="images"
                      style="display: none;">{{implode($images->pluck('url')->toArray(),',')}}</textarea>
            <div class="form-group-lg col-lg-6">
                <label for="LatLng">مختصات روی نقشه</label>
                ...
                <script>
                    ...
                    function init() {
                        var mapOptions = {
                            zoom: 16,
                            center: new google.maps.LatLng({!! $company->LatLng !!}),
                            styles: []
                        };
                        ...
                        var marker = new google.maps.Marker({
                            position: new google.maps.LatLng({!! $company->LatLng !!}),
                            map: map,
                            title: '{!! $company->name !!}'
                        });
                        var markers = [marker];
                ...
                {{ Form::text('LatLng', $company->LatLng, array('class' => 'form-control ltr-input')) }}
            </div>
