                        ...
                        <div class="form-group col-lg-4">
                            ...
                            <select name="company_id" id="company_id" class="form-control chosen">
                                ...
                                @foreach($companies as $id=>$company)
                                    <option value="{{$id}}" {{($_GET['company_id'] && $_GET['company_id'] ==$id?' selected ':'')}}>{{$company}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @foreach($tickets as $ticket)
                            ...
                            <tr class="gradeX">
                                ...
                                <td style="direction: ltr !important; text-align: right !important;">
                                    {{JDate::createFromCarbon(Carbon::parse($ticket->updated_at))->format('Y-m-d H:i:s')}}

                                </td>
                                <td style="color: {!! config('app.enum_ticket_status_colors')[$ticket->status] !!};">
                                    {{ config('app.enum_ticket_status')[$ticket->status] }}
                                </td>
                                <td style="color: {!! config('app.enum_ticket_priority_colors')[$ticket->priority] !!};">
                                    {{ config('app.enum_ticket_priority')[$ticket->priority] }}
                                </td>
                                <td>
                                    <a href="{{route('tickets.show',[$ticket->id])}}" class="btn btn-xs btn-outline btn-primary">مشاهده</a>
                                    @if($ticket->status != 'closed')
                                        <a class="btn btn-xs btn-outline btn-danger confirm_close_ticket" route="{!! route('tickets.close',[$ticket->id]) !!}">بستن</a>
                                    @endif
                                </td>
