        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="form-group-lg col-lg-6">
                    ...
                    <h1><i class='fa fa-user-md'></i>&nbsp;ویرایش دسته بندی</h1>
                    <hr>

                    {{ Form::model($galleryCategories, ['route' => ['video_gallery.update', $galleryCategories->id], 'method' => 'PUT','class'=>'form','files' => true]) }}
                    ...
                    <div class="form-group">
                        {{ Form::label('icon', 'آیکن') }}
                        <input name="icon" class="form-control icp icp-auto" value="{{$galleryCategories->icon}}" type="text" />
                    </div>

                    <div class="form-group">
                        {{ Form::label('name', 'نام') }}
                        {{ Form::text('name', $galleryCategories->name, array('class' => 'form-control')) }}
                    </div>
                    <div class="form-group">
                        {{ Form::label('parent_id', 'مجموعه') }}
                        <div style="color:red;padding:5px;">انتخاب مجموعه الزامی می باشد</div>
                        <select class="form-control" name="parent_id" id="parent_id">
                            <option value="" >انتخاب کنید</option>
                            @foreach(\HR\VideoGallery::all() as $cat)
                                @if($cat->id == $galleryCategories->id) @continue @endif
                                <option value="{{ $cat->id }}" {{ old('parent_id')? $cat->id==old('parent_id')?'selected':'': $cat->id==$galleryCategories->parent_id?'selected':''}}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="clearfix" style="margin: 300px"></div>
