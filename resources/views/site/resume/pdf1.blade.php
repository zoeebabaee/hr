...
<section class="clearfix container">
    ...
    <div class="clearfix right-sidebar">
        ...
        <div style="text-align: center;font-size: 27px;margin: 0px;">
            <strong>{{$resume->user->first_name.' '.$resume->user->last_name}}</strong>
        </div>
        <div class="r-section">
            <div class="r-title">
            ...
            <div class="r-content">
                <p><span class="line-height-28"><img src="https://people.golrang.com/resume/img/i1.png" alt="" title=""></span>
                    متولد {{\HR\myFuncs::farsi_numbers(str_replace('-','/',substr($resume->user->profile->born_date, 0, 4)))}}
                    ،
                ...
                @if($resume->user->profile->gender==1)
                    <p><span class="line-height-30"><img src="https://people.golrang.com/resume/img/i6.png" alt=""
                        ...
                        @if($resume->user->profile->military_status==1)
                            دارای پایان خدمت
                        @elseif($resume->user->profile->military_status==2)
                            دارای معافیت،
                            ...
                            @elseif($resume->user->profile->reason_exemption==4)
                                موارد خاص
                            @endif
                        @elseif($resume->user->profile->military_status==3)
                            خرید خدمت
                        @elseif($resume->user->profile->military_status==4)
                            خدمت سربازی مشمول
                        @endif
                    </p>
                @endif
                ...
                <ul class="ul-content">
                    @if($resume->educational_details && $resume->educational_details->count())
                        ...
                        @foreach($resume->educational_details->sortByDesc('grade') as $grade)
                                ...
                                <tr>
                                    <td>{{config('app.enum_last_degree')[$grade->grade]}}</td>
                                    <td>{{$grade->institute}}</td>
                                    <td>{{$grade->field}}</td>
                                    <td>{{\HR\myFuncs::farsi_numbers($grade->average)}}</td>
                                    <td>1393</td>
                                </tr>
                                <tr>
                                    <td>کارشناسی</td>
                                    <td>دانشگاه آزاد اسلامی</td>
                                    <td>کامپیوتر</td>
                                    <td>14</td>
                                    <td>{{\HR\myFuncs::farsi_numbers(substr($grade->start_date,0,4))}}
                                        @if($grade->end_date!="")
                                            تا
                        ...
                        @foreach($resume->work_experiences->sortByDesc('start_date') as $work)
                            ...
                            <span class="year" >
                                <img src="https://people.golrang.com/resume/img/calendar.png" alt=""
                                     title="">
                                {{\HR\myFuncs::farsi_numbers(substr($work->start_date,0,4))}}
                                @if(!is_null($work->end_date) && !empty($work->end_date))
                                    تا
                                    {{\HR\myFuncs::farsi_numbers(substr($work->end_date,0,4))}}
                                @else
                                    تاکنون
                                @endif
                            </span>
