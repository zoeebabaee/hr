...
<head>
    ...
    <link rel="stylesheet" href="https://people.golrang.com/resume/css/main-2.css?v=14">
    <style>
        img, p {
...
<section class="clearfix container">
    ...
    <div class="clearfix right-sidebar">
        ...
        <div class="r-section">
            ...
            <div class="r-content">
                <p>
                    <span class="line-height-28"><img src="https://people.golrang.com/resume/img/i1.png" alt=""
                                                      title=""></span>
                    {{$years_old}}
                    ساله{{--،--}}
                   متولد {{$resume->user->profile->born_date}}

                </p>
                <p>
                    <span class="line-height-28"><img src="https://people.golrang.com/resume/img/marriage.png" alt=""
                                                      title=""></span>
    ...
    <div class="wrapper-left-content">
                    ...
                    @if($resume->educational_details && $resume->educational_details->count())
                        <table>
                            ...
                            @foreach($resume->educational_details->sortByDesc('grade') as $grade)
                                ...
                                <tr>
                                    <td>{{$degrees_array[$grade->grade]}}</td>
                                    <td>{{$grade->institute}}</td>
                                    <?php if ($grade->field_id = 0): ?>
                                        <td>{{$grade->field_type}}</td>
                                    <?php else: ?>
                                        <td>@if($grade->grade == 1){{$grade->field_type}}@else{{$grade->field}}{{($grade->orientation == null)?'':' ('.$grade->orientation.')'}}@endif</td>
	                                <?php endif; ?>
                                    <td style="direction:ltr;">{{\HR\myFuncs::farsi_numbers($grade->average)}}</td>
                                    <td>
                                        @if($degrees_array[$grade->grade]!=="دیپلم")
        ...
        <div class="left-content">
            ...
            <div class="l-content">
                <ul class="ul-content-experience">
                    @if($resume->work_experiences && $resume->work_experiences->count())
                        @foreach($resume->work_experiences->sortByDesc('start_date') as $work)
                            ...
                            <span class="year" style="direction:rtl">
                                <img src="https://people.golrang.com/resume/img/calendar.png" alt="" title="">
                    ...
                    <table>
                        ...
                        @foreach($resume->foreign_languages as $lang)
                            <tr>
                                ...
                                @if(is_null($lang->certificate) || empty($lang->certificate))
                                    <td>-</td>
                                @else
                    ...
                    <ul class="ul-content">
                        <table>
                            ...
                            @foreach($resume->professional_training_records as $expert)
                                ...
                                <tr>
                                    <td>{{$expert->name}}</td>
                                    <td>{{$expert->pivot->institute_name}}</td>
                                    <td>{{\HR\myFuncs::farsi_numbers($expert->pivot->finish_year)}}</td>
                                    <td>{{\HR\myFuncs::farsi_numbers($expert->pivot->duration)}}</td>
                                </tr>
                            @endforeach
                        </table>
                    </ul>
                    ...
                    <ul class="ul-content">
                        <table>
                            ...
                            @foreach($resume->family as $family)
                                <tr>
                                    <td>{{$family->name}}</td>
                                    <td>{{config('gng_config.gng.relation')[$family->relation]['name']}}</td>
                                    @if($family->job)
                                        <td>{{$family->job}}</td>
                                        <td>{{$family->organization?$family->organization:'-'}}</td>
                                    @else
                                        <td>-</td>
                                        <td>-</td>
                                    @endif
