                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($messages as $message)
                            <tr>
                                ...
                                <td>
                                    {{$message->receiver_user->first_name.' '.$message->receiver_user->last_name}}
                                    @if($message->receiver_user->company->count())
                                        ({{$message->receiver_user->company->first()->name}})
                                    @endif
                                </td>
                                <td>
                                    {{$message->subject}}
                                ...
                                <td style="text-align: center;">
                                    <div style="float: right; margin: 2px;">
                                        {!! Form::open(['method' => 'POST', 'route' => ['user.messages.restore', $message->id] ]) !!}
                                        {!! Form::button('<i class="fa fa-undo"></i>', ['type'=>'submit','class' => 'btn btn-primary btn-sm']) !!}
                                        {!! Form::close() !!}
                                    </div>
                                    <div style="float: right; margin: 2px;" >
                                        <a href="{{route('user.messages.show',$message->id)}}"><button class="btn btn-primary btn-sm"><i class="fa fa-eye"></i></button></a>
                                    </div>
                                </td>
