...
{{ Form::open(array('method'=>'PUT','route' => 'user.resume.3.store','files' => true)) }}

<div class="" id="scroll-resume">
    @if (count($errors) > 0)
        ...
        <div class="container">
            <div class="col-md-4">
                <div class="people-forms-fields-group">
                    <input type="checkbox"
                           {{($resume->work_experiences->count() == 0 && HR\myFuncs::percent_state(Auth::user()->complete_percent)[3])?' checked ':''}} value="1"
                    id="no_work_experience" name="no_work_experience"
                    />
                    &nbsp;
        ...
        <div id="rows-div" class="container ">
            ...
            <div class="row col-12 add-experience">
                <div class="col-md-4">
                    <div class="people-forms-fields-group">
                        ...
                        <input maxlength="100" id="title" type="text"  class="form-control people-forms-fields" oninput="$('#title-help').text('')"  >
                        <label>شرکت</label>
                ...
                <div class="col-md-4">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <input maxlength="50" id="last_post" type="text" class="form-control people-forms-fields" oninput="$('#last_post-help').text('')"  >
                ...
                <div class="col-md-4">
                    <div class="row phone-Input">
                        <div class="people-forms-fields-group col-8 col-md-8 pl-0">
                            <input id="phone_number_ext" name="phone_number_ext" class="form-control people-forms-fields" maxlength="8" type="tel" onchange="$('#phone_number-help').text('')">
                            <label>تلفن</label>
                            <span id="phone_number-help" class="help-block"></span>
                        </div>
                        <div class="people-forms-fields-group col-4 col-md-4 pr-1">
                            <input id="phone_number_code" name="phone_number_code" class="form-control people-forms-fields" maxlength="3" type="tel" onchange="$('#phone_number-help').text('')" >
                        </div>
                ...
                <div class="row col-md-12 m-0 p-0">
                    ...
                    <div class="col-md-4">
                        <div class="people-forms-fields-group">
                            <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                            <select name="start_year" id="start_year" class="form-control chosen" onchange="$('#start_year-help').text('')" >
                                <option value=""></option>
                    ...
                    <div class="col-md-4">
                        <div class="people-forms-fields-group">
                            <input type="checkbox" value="1" id="until_now" name="until_now"  onchange="$('#end_month-help').text('');$('#end_year-help').text('')"/>
                            <label for="until_now">
                ...
                <div class="col-md-4">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <select name="end_month" id="end_month" class="form-control chosen"  onchange="$('#end_month-help').text('')" >
                            <option value=""></option>
                            @foreach(config('app.persian_months') as $key=>$item)
                            <option value="{{$key}}">{{$item}}</option>
                            @endforeach
                ...
                <div class="col-md-4">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <select name="end_year" id="end_year" class="form-control chosen" onchange="$('#end_year-help').text('')" >
                            <option></option>
                            @for($i = $current_year; $i > $current_year - 70; $i--)
                            <option value="{{$i}}">{{$i}}</option>
                            @endfor
                ...
                <div class="col-md-4">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <select  onchange="fillAreaCode()"  name="province" id="provinces" data-placeholder="یک گزینه انتخاب کنید"  class="ostan-select form-control has-value form-control people-forms-fields po-input-select chosen"  >
                            <option value="">یک گزینه را انتخاب کنید</option>

                            @foreach($provinces as $province)
                                <option value="{{$province->id}}" areacode="{{$province->areacode}}">{{(isset($province->name)?$province->name:'')}}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-12">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <textarea name="cause_interruption" id="cause_interruption" class="form-control form-control-area comment-contact" rows="3" onchange="$('#cause_interruption-help').text('')" placeholder="علت قطع همکاری"></textarea>
                ...
                <div class="col-md-12">
                    <div class="people-forms-fields-group">
                        <div class="fields-required"><img src="/site/default/Template_2019/img/Group 166.svg" /></div>
                        <textarea name="important_tasks" id="important_tasks" class="form-control form-control-area comment-contact" rows="4" onchange="$('#important_tasks-help').text('')" placeholder="اهم وظایف"></textarea>
                        <span id="important_tasks-help" class="help-block"></span>
                    </div>
                </div>
                <div class="mt-5 text-center btn add-row-experimental add-row-btn"><button type="button" class="save save-btn pr-5 pl-5" id="save_job"><i class="fa fa-plus"></i> افزودن سابقه شغلی </button></div>
            ...
            <fieldset class="mt-5 w-100">
                <legend class="text-left">
                    <input type="submit" {{--onclick="var job_id = $('#save_job').attr('job_id'); if (job_id != null) { $('#save_job').click(); }" --}} class="save send-btn-green" title="ذخیره و ادامه" value="<?= (Auth::user()->complete_percent != 31)?'ذخیره و ادامه':'ذخیره' ?>">
                </legend>
            </fieldset>
        </div>
    </form>
...
@section('script')
    ...
    <script>
        ...
        $('#workform').submit(function(e) {
            ...
            if (job_id != null) {
                ...
                if($('#title').val()!=""||$('#last_post').val()!=""||$('#cause_interruption').val()!=""||$('#important_tasks').val()!=""||$('#phone_number_ext').val()!=""||$('#phone_number_code').val()!=""){
                    if (confirm("سابغه ی شغلی شما ذخیره نشده است،آیا میخواهید ادامه دهید؟")) {
                        this.submit();
                    }else{
                        $('#save_job').click();
                    }
        ...
        $('body').on('click','.edit-item',function () {
            ...
            $('#provinces').val($('#provinces' + $(this).attr('id')).val());
                        ...
                        if (data != 0) {
                            ...
                            $('#rows-div').prepend(
                                '<div id="row_'+data+'" class="row edu-result-row prev-experiences" style="display:block;">' +
                                    ...
                                    '<input id="provinces_' + data + '" value="'+province_id+'" type="hidden"><' +
