...
@section('meta')
    ...
    <meta property="og:url" content="{{route('site.blog.show',$content->alias)}}"/>
...
--}}                            
        ...
        @section('script')
            <script src="/site/default/js/fingerprint.js"></script>

            <script>

                function getFP(id, value) {
                    new Fingerprint2().get(function (result, components) {
                        var info = {
                            fingerprint: result
                        };
                        sendVoteAjax(id, value, info.fingerprint);
                    });
                }

                function sendVoteAjax(id1, value1, fingerprint1) {
                    $.post("{{route('site.blog.vote')}}", {
                        id: id1,
                        value: value1,
                        ...
                        $('#like_box').text('مشکل در ثبت رای شما');
                    });
                }
                ...
                $('.detail-blog p').each(function () {
                    var temp = $(this).html();
                    if (temp && !is_persian(temp) && !temp.includes("type=\"image\"")) {
                        $(this).css("text-align", "left");
                        $(this).css("direction", "ltr");
                        $(this).css("font-family", "Calibri");
                        $(this).css("font-size", "15px");
                ...
                $('.detail-blog blockquote').each(function () {
                    var temp = $(this).html();
                    if (temp && !is_persian(temp)) {
                        $(this).css("text-align", "left");
                        $(this).css("direction", "ltr");
