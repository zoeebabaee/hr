    ...
    <div>
        {{--@if(!isset($web) || $web != true)--}}
            {{--<p style="direction: rtl;font-family: Tahoma;font-size: 12px">آیا تصاویر را نمی بینید؟ <a--}}
                        {{--href="{{route('emails.apply_reject', $apply->id)}}">اینجا کلیک--}}
                    {{--کنید...</a></p>--}}
        {{--@endif--}}
                            ...
                            <div class="main" style="background-color:rgb(255,255,255);display:block;"
                                ...
                                <div class="block-text" data-clonable="true"
                                    ...
                                    <p style="text-align:right; padding:14px; direction:rtl;margin-top:5px;margin-bottom:10px;">
                                        ...
                                        اولیه درخواست شما برای فرصت شغلی‌ «{{$apply->job->title}} در شرکت {{$apply->job->company->name}}» انجام شد
                                        و متاسفانه @if($apply->reject_reason) {{'به دلیل '.$apply->reason->reason}} @endif امکان همکاری در این مقطع زمانی میسر نمی باشد.امیدواریم
                                        بتوانیم در آینده نزدیک، زمینه پیوستن به خانواده گلرنگ را برایتان فراهم آوریم.
                                    </p>

                                    @if(!empty($apply->reject_description))
                                    <p style="text-align:right; padding:14px; direction:rtl;margin-top:5px;margin-bottom:5px;"><strong>توضیحات تکمیلی</strong></p>
                                    <pre style="background-color: #eeeeee;text-align:right; padding:14px; direction:rtl;margin-top:5px;margin-bottom:10px;font-family: Tahoma;font-size: 12px">{!! $apply->reject_description !!}</pre>
                                    @endif
